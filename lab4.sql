-- Create tables
DROP TABLE IF EXISTS assignments, projects, employees CASCADE;
...
SELECT first_name || ' ' || last_name
    AS full_name, department, salary
FROM employees;
...
-- Task 1.3
SELECT first_name || ' ' || last_name
    AS full_name, COALESCE(email, 'No email provided') AS email
FROM employees;
...
FROM employees WHERE salary BETWEEN 60000 AND 70000;

-- Task 2.3
SELECT first_name || ' ' || last_name AS full_name, last_name, department
FROM employees WHERE last_name LIKE 'S%' OR last_name LIKE 'J%';

-- Task 2.4
SELECT first_name || ' ' || last_name AS full_name, department, manager_id
FROM employees WHERE manager_id IS NOT NULL AND department = 'IT';
...
-- Task 3.1
SELECT UPPER(first_name || ' ' || last_name) AS full_name_upper, LENGTH(last_name) AS len,
       SUBSTRING(email FROM 1 FOR 3) AS email_prefix FROM employees;

-- Task 3.2
SELECT first_name || ' ' || last_name AS full_name, salary*12 AS annual,
       ROUND(salary/12,2) AS monthly, salary*0.10 AS raise FROM employees;

-- Task 3.3
SELECT FORMAT('Project: %s - Budget: $%s - Status: %s', project_name, budget, status) AS project_summary FROM projects;
...
-- Task 5.2
SELECT first_name || ' ' || last_name AS full_name FROM employees WHERE department='IT'
INTERSECT
SELECT first_name || ' ' || last_name AS full_name FROM employees WHERE salary>65000;

-- Task 5.3
SELECT employee_id, first_name || ' ' || last_name AS full_name FROM employees
EXCEPT
SELECT e.employee_id, e.first_name || ' ' || e.last_name FROM employees e JOIN assignments a ON e.employee_id=a.employee_id;
...
WHERE employee_id IN (SELECT a.employee_id FROM assignments a JOIN projects p ON a.project_id=p.project_id WHERE p.status='Active');
...
-- Task 7.2
SELECT p.project_name, SUM(a.hours_worked) AS total_hours, COUNT(DISTINCT a.employee_id) AS employee_count
FROM projects p JOIN assignments a ON p.project_id=a.project_id
GROUP BY p.project_name HAVING SUM(a.hours_worked)>150;

-- Task 7.3
SELECT e.department, COUNT(*) AS emp_count, AVG(e.salary) AS avg_salary,
       (SELECT e2.first_name || ' ' || e2.last_name FROM employees e2
        WHERE e2.department=e.department ORDER BY e2.salary DESC LIMIT 1) AS highest_paid,
       GREATEST(AVG(e.salary), MIN(e.salary)) AS salary_check,
       LEAST(MAX(e.salary), AVG(e.salary)) AS salary_compare
FROM employees e GROUP BY e.department;
